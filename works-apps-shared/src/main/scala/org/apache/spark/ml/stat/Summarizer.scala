...
/**
 * A builder object that provides summary statistics
 * about a given column.
 *
 * Users should not directly create such builders, but
 * instead use one of the methods in
 * [[Summarizer]].
 *
 * This functionality was added in v2.3.0 and is provided
 * here as CDAP Standalone does not support this version
 * yet.
 */
@Experimental
 ...
 *
 * The methods in this package provide various statistics
 * for Vectors contained inside DataFrames.
 *
 * This class lets users pick the statistics they would like
 * to extract for a given column. Here is
 * an example in Scala:
 * {{{
 ...
 * }}}
 *
 * Note: Currently, the performance of this interface is
 * about 2x~3x slower then using the RDD interface.
 *
 * This functionality was added in v2.3.0 and is provided
 * here as CDAP Standalone does not support this version
 * yet.
...
private[ml] object SummaryBuilderImpl extends Logging {
  ...
  private case class MetricsAggregate(
     ...
     * __KUP__
     *
     * Method modified to be compliant with version v2.1.3
     */
    override def update(state: SummarizerBuffer, row: InternalRow): Unit = {
